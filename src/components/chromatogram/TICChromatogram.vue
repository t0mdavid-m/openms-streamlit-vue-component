...
export default defineComponent({
  ...
  computed: {
    ...
    data(): Plotly.Data[] {
      ...
      // Selected feature trace (red line with actual feature intensity) - only show for MS1
      if (this.showFeatureTrace && this.selectedFeatureTraceData && this.msLevelFilter !== 'ms2') {
        traces.push({
  ...
  methods: {
    async renderPlot(): Promise<void> {
      try {
        ...
        const modeBarButtons: any[] = [
          ...
          {
            title: this.msLevelFilter === 'ms2'
              ? 'Feature Trace (MS1 only)'
              : (this.showFeatureTrace ? 'Hide Feature Trace' : 'Show Feature Trace'),
            name: 'toggleFeatureTrace',
            icon: {
              width: 960,
              height: 960,
              // Eye icon - visibility toggle (always show disabled/crossed-out icon for MS2)
              path: (this.showFeatureTrace && this.msLevelFilter !== 'ms2')
            ...
            click: () => {
              // Feature trace only available for MS1
              if (this.msLevelFilter !== 'ms2') {
    ...
    setMsLevelFilter(filter: 'all' | 'ms1' | 'ms2'): void {
      ...
      // Clear integration when switching to MS2 (features are MS1 only)
      if (filter === 'ms2') {
