...
<script lang="ts">
import { ref, defineComponent, onMounted, onUpdated, onUnmounted, onErrorCaptured } from 'vue'
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      Streamlit.events.addEventListener(Streamlit.RENDER_EVENT, (event): void => {
        const renderEvent = event as CustomEvent<RenderData>
        renderData.value = renderEvent.detail
        componentError.value = ''
      })
